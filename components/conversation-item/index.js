  ...
  methods: {
    ...
    deleteConversation() {
      wx.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            wx.tim.deleteConversation(this.data.conversation.conversationID);
            this.setData({
              conversation: {},
              xScale: 0,
            });
    ...
    muteNotifications() {
      if (this.data.conversation.type === 'C2C') {
        ...
        wx.tim.setMessageRemindType({
            userIDList: [this.data.conversation.userProfile.userID],
            messageRemindType: wx.TIM.TYPES.MSG_REMIND_ACPT_NOT_NOTE
          })
          .then((imResponse) => {
            this.setData({
              xScale: 0,
        ...
        if (this.data.showMute === '取消免打扰') {
          wx.tim.setMessageRemindType({
              userIDList: [this.data.conversation.userProfile.userID],
              messageRemindType: wx.TIM.TYPES.MSG_REMIND_ACPT_AND_NOTE
            })
            .then((imResponse) => {
              this.setData({
                xScale: 0,
